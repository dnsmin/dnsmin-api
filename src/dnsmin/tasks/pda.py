...
from dnsmin.lib.mail import EmailSendResult


@current_app.task(name=TaskEnum.APP_MAIL.value, label='App Mail',
...
def mail(individual_tasks: bool = True, **kwargs) -> EmailSendResult:
    ...
    if not individual_tasks:
        ...
        for to in kwargs['mail_to']:
            ...
            tasks.append(celery_app.send_task(TaskEnum.APP_MAIL_SEND.value, kwargs=send_kwargs))
...
@current_app.task(name=TaskEnum.APP_MAIL_SEND.value, label='App Mail Send',
...
@current_app.task(name=TaskEnum.APP_ALERT.value, label='App Alert')
def alert(msg: str, info: Any = None, title: str = None):
    ...
    from dnsmin.lib.notifications.events import AlertEvent
...
@current_app.task(name=TaskEnum.APP_TEST.value, label='App Test Task')
def test():
...
@current_app.task(name=TaskEnum.APP_TEST_MAIL.value, label='App Test Mail')
def test_mail():
    ...
    celery_app.send_task(TaskEnum.APP_ALERT.value, kwargs={
        'msg': 'App Mail Test',
        'info': 'This is a mailing system test.'
    })
    logger.warning(f'Finished mail test task.')


@current_app.task(name=TaskEnum.APP_TEST_EXCEPTION.value, label='App Test Exception')
def test_exception():
    """Throws an exception immediately to test global exception handling."""
    raise Exception('Test Exception')


@current_app.task(name=TaskEnum.APP_TEST_EXCEPTION_RETRY.value, label='App Test Exception Retry',
...
@current_app.task(name=TaskEnum.APP_TEST_DELAY.value, label='App Test Delay')
def test_delay():
